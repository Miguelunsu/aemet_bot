...
def main():
    ...
    try:
        ...
        max_temp_12h_estaciones = get_12h_values()
        ...
        datos_temp_estaciones = tmax_reader_todays_month(ruta_csv_tmax_mes_a_mes, mes_actual_str_number)

        # Para debugging y ver que los valores extremos se estan haciendo bien
        # max_temp_12h_estaciones["0009X"]["tamax"] = 88.8
        
        # max_absoluta_superada_estaciones: nested diccionario de bools: bool_est_extrem_12h, historic_day_tmax
        max_absoluta_superada_estaciones = abs_12h_comparer_tmax(max_temp_12h_estaciones, datos_temp_estaciones)
    
        # keys que contienen los idemas que superaron la T max
        idemas_tmax_mes_superada = []
        print("Estaciones que superaron su T máxima:")
        for key, valores in max_absoluta_superada_estaciones[0].items():
            if valores.get("Tmax_superada_mes") is True:
                ...
                if valores.get("Tmax_superada_abs") is True:
                    ...
                    print(f"    Info del día histórico{max_absoluta_superada_estaciones[1][key]}.")
                else:
                    print(f"    Info del día de hoy{max_temp_12h_estaciones[key]}.")
                    print(f"    Info del día histórico{max_absoluta_superada_estaciones[1][key]}.")
